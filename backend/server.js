...
// Endpoint pour obtenir une citation d'Alfred Jarry depuis Quotable
app.get('/quote', async (req, res) => {
    try {
        ...
        if (!data.results || data.results.length === 0) {
            return res.status(404).json({ error: "Aucune citation trouvée pour Alfred Jarry." });
        }
...
// Endpoint pour traduire une citation
